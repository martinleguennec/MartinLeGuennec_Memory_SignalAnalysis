...
\<bold\> The section that follows was not made for the memory \</bold\>
...
This time we lost a lot of variance explanation by suppressing the values because we went from a R2 at 0.82 to a R2 at 0.65. Therefore, we will do the sorting in two steps : before we suppress all the pvalues inferior to 0.10, them we will make a final sorting with the pvalues \< 0.05.
...
Thanks to this one more step, we managed to get back to a R2 of 0.81.
...
This graph is pretty big, I recommand visualizing it with 1000 width pts and 1500 height pts.
...
# 
# results.models$model <- factor(
#   results.models$model, 
#   levels = c("modlin", "lmmixt", "tree", "forest"),
#   labels = c("Régression multiple", "Modèle linéaire mixte", 
#              "Arbre de régression", "Forêt aléatoire"))

```

### Statistical comparisons

We want to compare our models to see if their predicition is statistically different from one. We do that thanks to the Kruskall-Wallis test

```{r}
# res.aov <- results.models %>%
#   group_by(session) %>%
#   kruskal_test(accuracy ~ model) %>%
#   add_significance()
# res.aov
# 
# results.models %>%
#   group_by(session) %>%
#   kruskal_effsize(accuracy ~ model, ci = TRUE)
```
...
# pwc <- results.models %>%
#   group_by(session) %>%
#   wilcox_test(accuracy ~ model, p.adjust.method = "bonferroni", 
#               detailed = TRUE) %>%
#   add_significance()
# 
# pwc
...
# results.models %>%
#   group_by(session) %>%
#   wilcox_effsize(accuracy ~ model, p.adjust.method = "bonferroni", ci = TRUE)
# 
# 
# results.models %>%
...
Now we are going to viauslize this

```{r}
# # Prepare the values for the statistical tests
# pwc <- pwc %>%
#   filter(p.adj < 0.05) %>%  # We only want the significant tests
#   add_y_position()
# # pwc$y.position <- c(0.07, 0.072, 0.07, 0.072, 0.074)
# 
...
# pwc <- results.models %>%
#   group_by(session) %>%
#   wilcox_test(accuracy ~ model, 
#               p.adjust.method = "bonferroni", conf.level = .95,
#               paired = FALSE) 
# 
# temp <- results.models %>%
#   filter(model == "Régression multiple" | model == "Arbre de régression") %>%
#   ungroup
# 
# wilcox.test(temp$accuracy ~ temp$model, conf.int = TRUE,
...
#   report
# 
# kruskal.test(results.models$accuracy ~ results.models$model) %>% 
#   unlist(.) %>%
#   report()
# 
# results.models %>%
#   group_by(session, model) %>%
#   get_summary_stats(accuracy, type = "median_iqr")
# 
# 
# kruskal.test(accuracy ~ model, data = results.models)
#   # report()
# 
# report(pwc)
# pwc
...
# Compare the models to the regressions

```{r}
# # Sélectionner les colonnes de mesure
# measure_cols <- colnames(session.1)[6:ncol(session.1)]
# 
# # Créer un dataframe vide pour stocker les résultats
# summary_table <- data.frame(Sujet = character(),
#                             Measure = character(),
#                             P_Value = character(),
#                             R2 = numeric(),
#                             stringsAsFactors = FALSE)
# 
# # Calculer les corrélations et ajouter les résultats au tableau récapitulatif
# for (measure in measure_cols) {
#   
#   correlation <- cor.test(session.1$MV, session.1[[measure]])
#   
#   pvalue <- ifelse(correlation$p.value < 0.001, "<0.001 *",
#                    ifelse(correlation$p.value < 0.05, 
#                           paste0(round(correlation$p.value, 3), " *"),
#                           paste0(round(correlation$p.value, 3))
#                           )
#                    )
#   
#   result_row <- data.frame(Sujet = "All data",
#                            Measure = measure,
#                            P_Value = pvalue,
#                            R2 = round(correlation$estimate^2, 3))
#   summary_table <- bind_rows(summary_table, result_row)
#   }
# 
# 
# # Calculer les corrélations par sujet et ajouter les résultats au tableau récapitulatif
# for (measure in measure_cols) {
#   for (sujet in unique(session.1$ID)) {
#     
#     data_subject <- session.1 %>%
#       filter(ID == sujet) %>%
#       ungroup()
#     
#     correlation_by_subject <- cor.test(data_subject$MV,
#                                        data_subject[[measure]])
# 
#     pvalue <- ifelse(correlation_by_subject$p.value < 0.001, "<0.001 *",
#                    ifelse(correlation_by_subject$p.value < 0.05, 
#                           paste0(round(correlation_by_subject$p.value, 3), " *"),
#                           paste0(round(correlation_by_subject$p.value, 3))
#                           )
#                    )
#     
#     
#     
#     result_row <- data.frame(Sujet = sujet,
#                              Measure = measure,
#                              P_Value = pvalue,
#                              R2 = round(correlation_by_subject$estimate^2, 3))
#     summary_table <- bind_rows(summary_table, result_row)
#   }
# }
# 
# # Afficher le tableau récapitulatif par sujet
# corr <- summary_table %>%
#   pivot_wider(names_from = Sujet, values_from = c(P_Value, R2))
# 
# corr <- corr[, c(1, 2, 10, 3, 11, 4, 12, 5, 13, 6, 14, 7, 15, 8, 16, 9, 17)]
# 
# 
# session.1 %>%
#   gather(-c(1:5), key = "var", value = 'value') %>%
#   ggplot(aes(x = value, y = MV)) +
#            facet_grid(~ var, scales = "free") +
#            geom_point()
# 
# 
# test <-  lm(estimate.2, data = as.tibble(session.1))
# report(test)
# 
# summary(test)
# 
# 
# lowCI <- confint(test)[[2]]
# highCI <- confint(test)[[4]]
# 
# summary(test)$coefficients[[2]]
# summary(test)$adj.r.squared
# summary(test)$coefficients[[8]]
# 
# for (measure in measure_cols){
#   lm.session1 <- lm(MV ~ measure, data = as.tibble(session.1))
#   
# }
# colum
```
...
# # summary(test)
# # 
# # prediction <- predict(test, session.1)
# # sqrt(mean((prediction - session.1$MV)^2))
# 
# 
# session.1 %>%
#   get_summary_stats(IMNF_VaLa, maxRMS_VaLa, type = "full")
# 
# 
# prediction <- predict(tree.session1, session.1)
# SSR <- sum((prediction - mean(session.1$MV))^2)
# SST <- sum((session.1$MV - mean(session.1$MV))^2)
# SSR/SST
# 
# 
# data.frame(as.data.frame(report(lm.session2)))
# 
# summary(lm.session2)
# rsq.rpart(tree.session2)
# tmp <- printcp(tree.session2)
# 1-tmp[,c(2,3,4)]
```
